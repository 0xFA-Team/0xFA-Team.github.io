...
在计算 **15*8+11** 的时候，他不会再重新计算 ** 15 \* 8 ** 和 **8+11**   会直接调用上次循环的结果dp[1][2]和dp[2][3]进行计算，最后得出来整个式子的结果。这个式子是第一个数据到第三个数据，那么就用dp[1][3]储存。以此类推，当处理四个数据时，调用前三个数据的结果。依次递归，所以空间需要很大，但是会大大的节约运算的时间。
...
那么k的左边的所有情况，也就是 ** data1 * data2 + data3 ** 的所有情况，包括：
...
当时在四个符号排列的时候，计算K的左边和右边的值并不是左边和右边计算出来的最后的值。只是左边和右边所有情况中的一种情况。

所以最后的四个符号排列计算最后的值的所有情况应该为![](http://z-onion.cc/9.png)中.
