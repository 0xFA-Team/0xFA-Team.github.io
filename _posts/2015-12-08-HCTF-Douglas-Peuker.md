...
从这个式子不难看出
	...
	sum = a*A[3](+-)(A+B+C+D+E+F)+b*A[3](+-)(A+B+C+D+E+F)         (A[3]为3的阶乘)
...
还是那个例子：
	...
	15*8+11+4   现在计算整个式子的值，当K在第二个+的地方。处理完  (15*8)+(11+4）以后的值为t
...
去掉中间的-号还有四个符号，即还可以填四个符号，如下：
	...
	|A |B |C |D |                *  +  +  *
	-------------                1  2  3  4
那么因为左右分开考虑计算，那么 x + 和 + x互不影响，也就是1，2和3，4互不影响。也就是把1放进A把2放进D是一样的，因为他们中间的-号是最低优先级。最后依旧算的是：
